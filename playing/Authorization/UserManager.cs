...
namespace playing.Authorization
{
    public class UserManager : IUserManager
    {
        ...
        private Lazy<UserInfoDto> UserInfo {
            get
            {
                if (_userInfo==null)
                    _userInfo = new Lazy<UserInfoDto>(_userInfoRequester.Get(_rawToken));
                return _userInfo;
            }
        } 
        ...
        public IPhoenixUser SetUser(JwtToken token, string rawToken)
        {
            ...
            if (token.sub != null) Claims = new ClaimsPrincipal(new HttpListenerBasicIdentity(token.sub, null)).Claims;
